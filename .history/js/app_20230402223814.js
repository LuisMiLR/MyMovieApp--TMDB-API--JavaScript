...
function showMovieDetails(id) {
  const url = BASE_URL + 'movie/' + id + '?' + API_KEY + '&language=fr-fr';

  fetch(url)
    .then(res => res.json())
    .then(data => {
      const { title, poster_path, vote_average, overview, credits } = data;
      ...
      const content = `
        <div class="movie-details">
          ...
          <div class="movie-info">
            ...
            <p><strong>Aperçu :</strong> ${overview}</p>
          </div>
        </div>
      `;

      myNav.innerHTML = content;
    });
}
